...
from USCell import SCell as USCell
...
unit = args.unit
layer = args.layer
keep_prob_train = args.dropout if args.dropout else 0.0
regu_strength = args.regularize if args.regularize else 0.0
seq_len = preprocessed_data.input_train.shape[1]

def create_model(dim_input,dim_output,unit,layer,keep_prob_train,seq_len):
	...
	for i in range(layer):
		...
		lstm_cell = USCell(unit,dim_input)
		model.add(tf.keras.layers.RNN(lstm_cell,return_sequences=True,name='LSTM{}'.format(i)))
		model.add(tf.keras.layers.Dropout(keep_prob_train))
...
model = create_model(dim_input,dim_output,unit,layer,keep_prob_train,seq_len)


model.summary()

wd = os.getcwd()
dirname = 'train_simple_vel_partial_uscell_e{}lr{}seed{}unit{}layer{}prepend{}dropout{}regu{}'.format(args.epoch,
                                                               args.lr,
                                                               seed,
                                                               args.unit,
                                                               args.layer,
...
checkpoint_path = os.path.join(wd, dirname, "e{epoch:d}_model_c{loss:.4e}_cv{val_loss:.4e}.ckpt")
